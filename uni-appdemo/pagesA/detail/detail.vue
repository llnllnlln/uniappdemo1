<template>
	<view class="detail">
		<up-navbar title="" bg-color="#88088000" :autoBack="true" left-icon-color="#fff" />
		<view class="d-con">
			<image :src="details.dt.img" mode="aspectFill"></image>
			<view class="d-content">
				<view class="j-con">
					<view class="tit" style="display: flex;">
						<text style="margin-rigit:17px;font-size:17px">{{ details.dt.title }}</text>
						<up-tag text="课程内容" size="mini" shape="circle"></up-tag>
					</view>
					<view class="jj">
						<view style="font-weight:700;font-size:14px">课程介绍</view>
						<view class="nr">{{ details.dt.introduce }}</view>
					</view>
					<view class="j-con">
						<view class="jj">
							<text style="font-weight:700;font-size:14px">课程时长</text>
							<text class="nr">{{ details.dt.times }}</text>
						</view>
					</view>
					<view class="j-con ls">
						<view class="tit" style="font-size: 34rpx">热门课程</view>
...
<script setup>
	...
	onLoad((opt)=>{
		...
		console.log('详情111', JSON.parse(decodeURIComponent(opt.item)))
